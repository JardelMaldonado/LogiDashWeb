...
import {RankingMotoristasProps, TooltipMotoristasProps } from "../../types/dashboard/ranking";


function CustomTooltip({ active, payload }: Readonly<TooltipMotoristasProps>) {
...
export function RankingMotoristas({ dados }: Readonly<RankingMotoristasProps>) {
  return (
    <div className="bg-[#1E293B] rounded-2xl shadow-xl border border-slate-800 p-6 mb-8">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={dados} layout="vertical" margin={{ left: 0, right: 40, top: 0, bottom: 0 }}>
          ...
          <YAxis type="category" dataKey="nome" width={140} tick={{ fontSize: 11, fill: '#cbd5e1', fontWeight: 500 }} axisLine={{ stroke: '#334155' }} tickLine={false} />
          <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.02)' }} />
          <Bar dataKey="litros" fill="#8b5cf6" radius={[0, 6, 6, 0]} barSize={24} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
